...
public class CarController : MonoBehaviour
{
    ...
    public string roadTag = "road"; // 道路のタグ
    public float slowdownFactor = 0.5f; // 減速係数
    public float raycastDistance = 2f; // レイキャストの距離

    //キー入力
    GetBottons input = GetBottons.Instance;
    ...
    public void FixedUpdate()
    {
        ...
        //bool Button = input.GetButtonDown(GetBottons.Button.LEFT);
        //Debug.Log("left"　+ Button);

        //float motor = maxMotorTorque * input.GetRightTrigger();
        //float steering = maxSteeringAngle * input.GetRightStick().x;
        ...
        // 速度を時速に変換する
        // 車の速度をメートル/秒で取得
        motor = GetComponent<Rigidbody>().velocity.magnitude;
        // メートル/秒を時速（km/h）に変換
        int speedKMH = (int)(motor * 3.6f);
        // 速度の表示
        textComponent.text = speedKMH.ToString() + " km/h";
        //textComponent.text = Button.ToString();
    ...
    // 横転から復帰させる
    public void RecoveringFromRollover()
    {
